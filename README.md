...
## Authorization.php
This file is designed to authorize users by checking their login credentials against a MySQL database of user information. Once the user is validated, their username and password are stored in session variables, which can be used to authenticate the user on subsequent protected pages. The authorisation layer also includes role-based access control, allowing different levels of access to be granted based on user roles and functions.To use this authorisation layer, the user first logs in on the login page, which validates their credentials and redirects them to the protected page if successful. The session variables are then used to authenticate the user on subsequent pages, and the authorisation layer checks the user's role to determine which actions they are allowed to perform.This file could be further extended by adding additional roles and permissions, such as read-only access for certain users or the ability to grant access to specific pages or functions based on user roles. Additionally, the authorisation layer could be made more robust by implementing more secure password hashing algorithms and preventing SQL injection attacks by using prepared statements.
